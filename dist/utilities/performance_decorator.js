...
function MeasurePerformance(stageCategory = 'service') {
    return function (target, propertyKey, descriptor) {
        ...
        descriptor.value = async function (...args) {
            ...
            try {
                ...
                if (this.logger) {
                    this.logger.info(`[Internal Performance Audit] ${stageCategory} execution took ${durationMs}ms`);
                }
                ...
                const durationMs = Number((performance.now() - start).toFixed(2));
                if (this.logger) {
                    this.logger.error(`[Internal Performance Audit] ${stageCategory} failed after ${durationMs}ms`);
                }
                throw error;
            }
        };
